...
import { DataHealthSkeleton } from '../components/layout/ViewSkeletons';
...
export default function DataHealthPage() {
  ...
  return (
    <Skeleton
      name="data-health"
      loading={isLoading || !dataHealth}
      fallback={<DataHealthSkeleton />}
    >
      <div className="p-6 lg:p-8 space-y-6 max-w-6xl mx-auto font-sans animate-fade-in">
        {/* Header */}
        <div className="animate-fade-in-up">
          <h1 className="text-2xl font-bold text-[#EFEFEF] tracking-tight">{t('dataHealth.heading')}</h1>
        ...
        <div className="bg-[#16161A] border border-[#24242A] p-5 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-sm animate-fade-in-up stagger-1">
          ...
          <div className="text-[11px] text-[#777777] bg-[#121215] border border-[#24242A] px-3 py-1.5 rounded-lg">
            Last Sync: <strong className="text-[#CCCCCC]">{dataHealth?.system_time_utc || 'Live'}</strong>
          </div>
        </div>

        {/* Clean Telemetry Feeds Table */}
        <div className="bg-[#16161A] border border-[#24242A] rounded-2xl overflow-hidden shadow-sm animate-fade-in-up stagger-2">
