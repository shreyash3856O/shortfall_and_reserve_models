...
import { RiskSkeleton } from '../components/layout/ViewSkeletons';
...
export default function RiskRootCausePage({ selectedMineId = 'MN01' }: RiskRootCausePageProps) {
  ...
  return (
    <Skeleton
      name="risk-root-cause"
      loading={isLoading || !detail}
      fallback={<RiskSkeleton />}
    >
      <div className="p-6 lg:p-8 space-y-6 max-w-6xl mx-auto font-sans animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 animate-fade-in-up">
          <div>
            <h1 className="text-2xl font-bold text-[#EFEFEF] tracking-tight">{t('risk.heading')}</h1>
        ...
        {detail && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-7 bg-[#16161A] border border-[#24242A] p-5 space-y-4 rounded-2xl shadow-sm animate-fade-in-up stagger-1">
              <div className="flex justify-between items-center border-b border-[#222228] pb-3 text-[12px]">
              ...
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={detail.shap_breakdown} layout="vertical" margin={{ top: 10, right: 30, left: 60, bottom: 5 }}>
                    <CartesianGrid stroke="#202026" strokeDasharray="3 3" horizontal={false} />
                    <XAxis type="number" stroke="#333333" tick={{ fill: '#666666', fontSize: 10 }} tickLine={{ stroke: '#222228' }}
                      label={{ value: '+Risk Contribution (%)', position: 'insideBottom', fill: '#666666', fontSize: 10 }} />
                    <YAxis type="category" dataKey="feature" stroke="#333333" tick={{ fill: '#CCCCCC', fontSize: 11 }} tickLine={{ stroke: '#222228' }} width={140} />
                    <Tooltip contentStyle={{ backgroundColor: '#16161A', borderColor: '#26262E', borderRadius: '8px', fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: '12px' }} />
            ...
            <div className="lg:col-span-5 space-y-4 text-[12px] animate-fade-in-up stagger-2">
              {/* Diagnosis Card */}
              <div className="bg-[#16161A] border border-[#24242A] p-5 space-y-3 rounded-2xl shadow-sm">
                ...
                <div className="pt-3 border-t border-[#222228] space-y-2 text-[#888888]">
                ...
                <div className="space-y-2 text-[#CCCCCC]">
                  {detail.recommended_actions.map((act, idx) => (
                    <div key={idx} className="flex gap-2.5 text-[12px] items-start">
                      <span className="w-5 h-5 rounded bg-[#202026] border border-[#2E2E38] text-[#C0BDB8] font-bold text-[10px] flex items-center justify-center flex-shrink-0 mt-0.5">
                        {idx + 1}
                      </span>
